...
WITH savings AS (
    SELECT s.owner_id,
           ...
           SUM(s.confirmed_amount) AS savings_total  -- total amount in kobo
...
investments AS (
    SELECT s.owner_id,
           ...
           SUM(s.confirmed_amount) AS investment_total  -- total amount in kobo
    FROM savings_savingsaccount s
    JOIN plans_plan p ON s.plan_id = p.id
    WHERE p.is_a_fund = 1
    GROUP BY s.owner_id
...
SELECT
    ...
    s.savings_count,
    i.investment_count,
    ROUND((s.savings_total + i.investment_total) / 100, 2) AS total_deposits --convert kobo to naira
FROM savings s
